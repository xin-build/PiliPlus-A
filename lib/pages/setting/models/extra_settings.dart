...
List<SettingsModel> get extraSettings => [
  if (PlatformUtils.isDesktop) ...[
    ...
    NormalModel(
      title: '缓存路径',
      getSubtitle: () =>
          '$downloadPath（${_androidVideoCacheCodename(downloadPath)}）',
      leading: const Icon(Icons.storage),
  ...
  const SwitchModel(
    title: '启用AI原声翻译',
    subtitle: '在视频播放器中支持哔哩哔哩 AI 多语言原声翻译音轨选择与切换',
    leading: Icon(Icons.translate),
    setKey: SettingBoxKey.enableAiTranslate,
    defaultVal: true,
  ),
  const SwitchModel(
    title: '消息页禁用"收到的赞"功能',
...
Future<String> _androidStorageLabel(
  ...
  if (uuid != null && uuid.isNotEmpty) {
    return isDefault ? '外部存储（默认位置，$uuid）' : '外部存储（$uuid）';
  }
