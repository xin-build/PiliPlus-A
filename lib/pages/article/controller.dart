...
class ArticleController extends CommonDynController {
  ...
  List<SourceModel> images() {
    ...
    final list = <SourceModel>[];
    final topPics = opusData?.modules.moduleTop?.display?.album?.pics;
    if (topPics != null) {
      for (final p in topPics) {
        if (p.url != null && p.url!.isNotEmpty) {
          list.add(SourceModel(url: p.url!));
        }
      }
    }
    if (opus != null) {
      for (final e in opus!) {
        if (e.paraType == 2 && e.pic?.pics != null) {
          for (final p in e.pic!.pics!) {
            if (p.url != null && p.url!.isNotEmpty) {
              list.add(SourceModel(url: p.url!));
            }
          }
