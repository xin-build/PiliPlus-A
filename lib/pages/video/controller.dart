...
class VideoDetailController extends GetxController
  ...
  final isLoginVideo =
      Accounts.get(AccountType.video).isLogin || Accounts.main.isLogin;
  ...
  VideoItem findVideoByQa(int qa, {bool setCodecs = false}) {
    ...
    final candidateList = videoList.isNotEmpty ? videoList : allVideos;
    ...
    if (setCodecs) {
      if (bestIndex < preferCodecs.length) {
        ...
        final firstCodecs = candidateList.first.codecs;
        if (firstCodecs != null && firstCodecs.isNotEmpty) {
          currentDecodeFormats = VideoDecodeFormatType.fromString(firstCodecs);
        }
      }
    }

    return bestVideo ?? candidateList.first;
  }
  ...
  Future<void> changeVideoQuality(VideoQuality newQa) async {
    if (currentVideoQa.value == newQa) return;

    if (newQa.code > 80 && !isLoginVideo) {
      SmartDialog.showToast('该画质需要登录账号才可使用');
      return;
    }

    final isWiFi = await ConnectivityUtils.isWiFi;

    // 如果当前 Dash 列表已经包含目标画质，直接内存切换
    final hasQualityInMemory =
        data.dash?.video?.any((e) => e.quality.code == newQa.code) ?? false;
    if (hasQualityInMemory) {
    ...
    // 否则从网络重新请求对应画质流
    SmartDialog.showLoading(msg: '正在切换画质...');
    try {
      ...
      if (result case Success(:final response)) {
        ...
        if (videoList == null || videoList.isEmpty) {
          SmartDialog.showToast('获取画质失败，视频资源不存在');
          return;
        }
        ...
        final bool hasTarget =
            videoList.any((e) => e.quality.code == newQa.code);
        if (hasTarget) {
          plPlayerController.cacheVideoQa = newQa.code;
          currentVideoQa.value = newQa;
          updatePlayer();
          ...
          // 调用失败/未下发对应清晰度：自动提示并降级
          final fallbackQa = videoList.first.quality;
          SmartDialog.showToast(
            '获取 ${newQa.desc} 失败，已自动降级为 ${fallbackQa.desc}',
          );
          plPlayerController.cacheVideoQa = fallbackQa.code;
          currentVideoQa.value = fallbackQa;
          updatePlayer();
          if (!plPlayerController.tempPlayerConf) {
            setting.put(
              isWiFi
        ...
        SmartDialog.showToast('切换画质失败: $errMsg，已保持当前画质');
      }
    } catch (e) {
      SmartDialog.dismiss();
      SmartDialog.showToast('切换画质失败: $e');
    }
  }
  ...
  void setLanguage(String language) {
    ...
    if (!isLoginVideo && !Pref.trialVipQuality) {
  ...
  Future<void> _queryVideoUrl(bool fromReset, bool autoFullScreenFlag) async {
    if (cid.value == 0) {
      if (bvid.isNotEmpty || aid != 0) {
        ...
        if (res?.cid != null) {
          ...
          if (introRes case Success(:final response)) {
            if (response.pages?.isNotEmpty == true &&
                response.pages!.first.cid != null) {
              cid.value = response.pages!.first.cid!;
            }
          }
        }
      }
    ...
    if (result case Success(:final response)) {
      ...
      // 根据画质选编码格式
      if (supportFormats != null && supportFormats.isNotEmpty) {
        final formatItem = supportFormats.firstWhere(
          (e) => e.quality == targetVideoQa,
          orElse: () => supportFormats.first,
        );
        final itemCodecs = formatItem.codecs;
        if (itemCodecs != null && itemCodecs.isNotEmpty) {
          currentDecodeFormats = VideoUtils.selectCodec(itemCodecs, preferCodecs);
        }
      }
      ...
      final videosList = videoList
          ...
          .toList();
      final candidates = videosList.isNotEmpty ? videosList : videoList;

      /// 取出符合当前解码格式的videoItem
      firstVideo = candidates.firstWhere(
