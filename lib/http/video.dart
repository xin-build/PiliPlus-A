...
import 'package:PiliPlus/grpc/bilibili/main/community/reply/v1.pb.dart'
    show ReplyInfo;
import 'package:PiliPlus/grpc/playerunite.dart';
import 'package:PiliPlus/grpc/playurl.dart';
...
abstract final class VideoHttp {
  ...
  static Future<LoadingState<PlayUrlModel>> videoUrl({
    ...
    int effectiveCid = cid;
    final aid = avid ?? (bvid != null ? IdUtils.bv2av(bvid) : null);
    if (effectiveCid == 0 && (bvid != null || aid != null)) {
      final bvidStr = bvid ?? IdUtils.av2bv(aid!);
      final res = await SearchHttp.ab2cWithDimension(aid: aid, bvid: bvidStr);
      if (res?.cid != null) {
        effectiveCid = res!.cid;
      } else {
        final introRes = await VideoHttp.videoIntro(bvid: bvidStr);
        if (introRes case Success(:final response)) {
          effectiveCid = response.pages?.firstOrNull?.cid ?? 0;
        }
      }
    }

    if (effectiveCid == 0) {
      return const Error('无法获取有效的视频 CID');
    }

    if (Pref.trialVipQuality && (language == null || language.isEmpty)) {
      if (aid != null) {
        try {
          final uniteRes = await PlayerUniteGrpc.playViewUnite(
            aid: aid,
            cid: effectiveCid,
            qn: qn,
            isNeedTrial: true,
            fromScene: videoType == VideoType.pgc ? 'pgc' : 'ugc',
            extraContent: {
              if (epid != null) 'ep_id': '$epid',
              if (seasonId != null) 'season_id': '$seasonId',
            },
          );
          if (uniteRes case Success(:final response)) {
            if (response.dash?.video?.isNotEmpty == true ||
                response.durl?.isNotEmpty == true) {
              return Success(response);
            }
          }
          if (videoType == VideoType.ugc) {
            final grpcRes = await PlayUrlGrpc.playView(
              aid: aid,
              cid: effectiveCid,
              qn: qn,
              fourk: true,
            );
            if (grpcRes case Success(:final response)) {
              if (response.dash?.video?.isNotEmpty == true ||
                  response.durl?.isNotEmpty == true) {
                return Success(response);
              }
