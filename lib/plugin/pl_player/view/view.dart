  ...
  Widget buildBottomControl(
    ...
    Widget progressWidget(
      ...
      BottomControlType.aiTranslate => Obx(
        () {
          if (!Pref.enableAiTranslate) return const SizedBox.shrink();
      ...
      BottomControlType.speed => Obx(
        () => GestureDetector(
          onLongPress: () {
            ...
            final target = (current == 1.0) ? 2.0 : 1.0;
            plPlayerController.setPlaybackSpeed(target);
            SmartDialog.showToast('已切换为 ${target}X 倍速');
          },
      ...
      BottomControlType.qa => Obx(
        () {
          ...
          final isLogin = videoDetailController.isLoginVideo;
          final trialVipQuality = Pref.trialVipQuality;
          final availableQa =
              videoInfo.dash?.video?.availableVideoQualities ?? const <int>[];
            ...
            itemBuilder: (context) {
              return List.generate(
                ...
                (index) {
                  ...
                  final isVipQuality = (item.quality ?? 0) > 80;
                  final enabled = availableQa.contains(item.quality) ||
                      (isVipQuality && trialVipQuality && isLogin);
                  return PopupMenuItem<int>(
                    enabled: enabled,
                    height: 35,
                    ...
                    onTap: () async {
                      ...
                      if (isVipQuality && !isLogin) {
                        SmartDialog.showToast('该画质需要登录账号才可使用');
                        return;
                      }
                      final int quality = item.quality!;
                      final newQa = VideoQuality.fromCode(quality);
                      await videoDetailController.changeVideoQuality(newQa);
                    },
                    child: Text(
                      (isVipQuality && isLogin && trialVipQuality)
                          ? '${item.newDesc} (试看)'
                          : (item.newDesc ?? ''),
                      style: enabled
    ...
    final List<BottomControlType> userSpecifyItemRight = [
      ...
      if (flag) .fit,
      if (isNotFileSource && Pref.enableAiTranslate) .aiTranslate,
      .subtitle,
      .speed,
      if (isNotFileSource && flag) .qa,
      if (!plPlayerController.isDesktopPip) .fullscreen,
    ];
    final customBottomBarSort = Pref.playerBottomBarSort;
    if (customBottomBarSort != null && customBottomBarSort.isNotEmpty) {
      userSpecifyItemRight.retainWhere((e) => customBottomBarSort.contains(e.index));
      userSpecifyItemRight.sort(
        (a, b) => customBottomBarSort.indexOf(a.index).compareTo(customBottomBarSort.indexOf(b.index)),
      );
    }
    return PlayerBar(
      children: [
                    ...
                    child: Offstage(
                      ...
                      child: DecoratedBox(
                        ...
                        child: ComBtn(
                          ...
                          onLongPress: !isLive ? screenshotWebp : null,
                          ...
                          Expanded(
                            child: TweenAnimationBuilder<double>(
                              ...
                              child: BackwardSeekIndicator(
                                duration:
                                    plPlayerController.backwardDuration,
                          ...
                          Expanded(
                            child: TweenAnimationBuilder<double>(
                              ...
                              child: ForwardSeekIndicator(
                                duration:
                                    plPlayerController.forwardDuration,
                                onSubmitted: (Duration value) {
                                  plPlayerController
