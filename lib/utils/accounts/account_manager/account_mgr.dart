...
class AccountManager extends Interceptor {
  ...
  void onRequest(RequestOptions options, RequestInterceptorHandler handler) {
    ...
    if (isApp && options.responseType == ResponseType.bytes) {
      if (options.extra['phone_headers'] == true) {
        options.headers.addAll(GrpcHeaders.phoneHeaders(account.accessKey));
      } else {
  ...
  static Account _findAccount(String path) {
    final cleanPath = path.startsWith(HttpString.appBaseUrl)
        ? path.substring(HttpString.appBaseUrl.length)
        : path;
    if (ApiType.loginApi.contains(path) ||
        ApiType.loginApi.contains(cleanPath)) {
      return AnonymousAccount();
    }
    final accountType = AccountType.values.firstWhere(
      (i) {
        final set = ApiType.apiTypeSet[i];
        return set?.contains(path) == true || set?.contains(cleanPath) == true;
      },
      orElse: () => AccountType.main,
    );
    final account = Accounts.get(accountType);
    if (!account.isLogin && Accounts.main.isLogin) {
