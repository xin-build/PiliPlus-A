...
| 工具 / SDK | 建议版本 | 适用平台 |
| :--- | :--- | :--- |
| **Flutter SDK** | **3.47.0 (Stable)** | 全平台通用 |
...
只需推送符合规范的版本标签（如 `v2.1.3`、`release-2.1.3`），CI 会自动触发并在编译成功后把全平台 11 个安装包一并关联至该 Release：
...
1. **`flutter-action` 版本下载报错**：
   - 请保持 `pubspec.yaml` 中 `environment.flutter: 3.47.0` 精确版本号写法，勿加 `>=` 或 `^` 符号。
2. **`patch.ps1` 提示找不到 cupertino_ui**：
